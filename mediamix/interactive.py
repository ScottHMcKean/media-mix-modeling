...
def _plot_geometric_adstock_curve(alpha):
    """Plot geometric adstock curve for visualization.
...
def _plot_saturation_curve(mu):
    """Plot saturation curve for visualization.

    :param mu: Saturation rate parameter
    """
...
def display_geometric_adstock_and_delay_interactive():
    """Display interactive widgets for exploring adstock and saturation effects."""
    style = {"description_width": "initial"}
    widgets.interact(
        _update_geometric_adstock_and_delay_experiment,
        saturation_mu=widgets.FloatSlider(
            min=0.1,
            max=2.0,
            step=0.1,
            value=1.0,
        ...
        adstock_alpha=widgets.FloatSlider(
            min=0,
            max=1,
            step=0.1,
            value=0.5,
