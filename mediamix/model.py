...
def log_figure(filename):
    """Context manager for logging figures to MLflow.
...
class Channel:
    ...
    def __init__(
        ...
        :param channel_name: Name of the media channel
        :param beta_sd_prior: Standard deviation of beta prior
        :param has_saturation: Whether to include saturation effect
        :param saturation_alpha_prior: Alpha parameter for saturation prior
        :param saturation_beta_prior: Beta parameter for saturation prior
        :param has_geometric_adstock: Whether to include adstock effect
        :param geometric_adstock_alpha_prior: Alpha parameter for adstock prior
        :param geometric_adstock_beta_prior: Beta parameter for adstock prior
        """
    ...
    def get_contribution(self, obs: np.ndarray):
        """Calculate the contribution of this channel to the model.

        :param obs: Observed values for this channel
        :return: Channel contribution
        """
        out = obs
        ...
        channel_b = pm.HalfNormal(f"beta_{self.channel_name}", sigma=self.beta_sd_prior)
...
class ModelConfig:
    ...
    def __init__(
        ...
        """Initialize a media mix model configuration.

        :param outcome_name: Name of the outcome variable
        :param intercept_mu: Mean of intercept prior
        :param intercept_sd: Standard deviation of intercept prior
        :param sigma_beta: Beta parameter for sigma prior
        :param outcome_scale: Scale factor for outcome variable
        :param channels: List of media channels
        """
    ...
    def add_channel(self, channel):
        """Add a media channel to the model.
    ...
    def create_model(self, df):
        """Create a PyMC model from the configuration.

        :param df: DataFrame containing the data
        :return: PyMC model instance
        """
        model = pm.Model()
        ...
        with model:
            ...
            intercept = pm.Normal(
                "intercept", mu=self.intercept_mu, sigma=self.intercept_sd
            )
            ...
            for var_name in self.index_vars:
                ...
                ind_beta = pm.Normal(f"beta_{var_name}", sigma=0.5, shape=shape)
    ...
    def to_config_dict(self):
        ...
        config = {
            "outcome": {
                ...
                "intercept": {"mu": self.intercept_mu, "sd": self.intercept_sd},
    ...
    def scale_data(self, df):
        ...
        :return: Tuple of (scalers, scaled DataFrame)
        """
    ...
    def run_inference(self, params, df):
        ...
        :param params: Dictionary of inference parameters
        :param df: DataFrame containing the data
        :return: Tuple of (model, inference data, scalers)
        """
    ...
    def log_results(self, df, idata):
        ...
        :param df: DataFrame containing the data
        :param idata: Inference data from PyMC
        """
